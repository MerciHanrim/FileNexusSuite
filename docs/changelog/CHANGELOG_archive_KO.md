...
### Changed
- **번역이 Qt Linguist `.ts/.qm` 런타임으로 마이그레이션됨 (v1.1.0 모듈화 트랙 — 국제화)** — `FileNexusSuite.py` 내부에 인라인으로 정의되어 있던 `TRANSLATIONS` 딕셔너리가 Qt Linguist `.ts` (소스) 및 `.qm` (런타임) 파일로 컴파일됨. 소스 파일은 `translations_ts/` 아래 5개 언어(ko/en/ja/zh_cn/zh_tw)별로, 런타임 파일은 `translations/` 아래에 존재하며, `scripts/` 아래의 헬퍼 스크립트가 lupdate/lrelease 사이클을 자동화 (`scripts/update_translations.bat`, `scripts/add_ts_contexts.py`). 이전에 `_t(key)`로 가져오던 모든 UI 문자열은 이제 `self.tr(key)`로 노출되고, 런타임에 `QTranslator`를 통해 해석됨. 25개 컨텍스트에 걸쳐 503개 번역 확정 (BatchRenamerPanel/FileNexusSuite/TextMergerPanel/TagEditorPanel/TextFixerPanel이 가장 큰 컨텍스트). 언어는 첫 실행 시 OS 설정에서 자동 감지되며 설정에서 변경 가능. 번역 협업은 `.ts` 파일을 Qt Linguist로 직접 편집하며, UI 변경 후 `scripts/update_translations.bat`로 `.qm` 런타임 파일을 재생성. 모듈화 트랙 초기에 `TRANSLATIONS` 딕셔너리를 `fns_translations.py` 모듈 + 언어별 Python 패키지로 분리하는 탐색적 접근이 있었으나, 이 Qt Linguist 마이그레이션으로 대체됨. 데이터 계층은 이제 Python 소스 외부에 존재.
- **테마 시스템이 `fns_theme.py`로 분리됨 (v1.1.0 모듈화 트랙 — 테마 시스템 분리)** — `FileNexusSuite.py` 내부에 인라인으로 정의되어 있던 9-테마 팔레트 dict(`THEMES`), 5개 테마 헬퍼(`_hex_rgba`, `_combo_arrow_url`, `_detect_system_theme`, `_resolve_theme`, `make_style`), 그리고 `_apply_card_shadow` 위젯 헬퍼가 신규 `fns_theme.py` 형제 모듈로 분리됨(469줄, 9 테마 × 18 키). 새 모듈은 `_t()` / TRANSLATIONS / ConfigManager / fns_utils 의존이 없음 — PySide6 primitives(`QApplication`, `QPalette`, `QColor`, `QGraphicsDropShadowEffect`)만 import하며 본체 런타임 상태와 독립적으로 재사용 가능. `_apply_card_shadow`는 시그니처가 `_apply_card_shadow(widget)` → `_apply_card_shadow(widget, border_color)`로 변경 — `BORDER` global 의존을 명시적 인자로 풀어, 순환 import 위험을 차단하고 `fns_theme.py`를 자기-완결로 유지. 본체 모듈의 런타임 globals에 결합된 함수들(`_unpack`, `STYLE`, `_T`, `_accent_alpha`)은 헬퍼 함수 분리 단계의 Text Converter 헬퍼와 동일 패턴으로 본체에 그대로 둠. 본체 라인 수는 15,519 → 15,130 (-389줄); 새 `fns_theme.py`는 469줄(분리 과정에서 시그니처 변경된 `_apply_card_shadow`가 한 헬퍼 추가됨). `FileNexusSuite.py`의 호출 자리는 `fns_utils` import 직후에 단일 `from fns_theme import (THEMES, _hex_rgba, _combo_arrow_url, _detect_system_theme, _resolve_theme, make_style, _apply_card_shadow,)` 블록을 통해 동일한 식별자를 그대로 사용. 모듈화 트랙의 두 번째 단계; 여기서 분리한 시각 스타일 계층은 위의 국제화 마이그레이션으로 자연스럽게 보완됨.
- **헬퍼 함수들이 `fns_utils.py`로 분리됨 (v1.1.0 모듈화 트랙 — 헬퍼 함수 분리)** — `FileNexusSuite.py` 내부에 인라인으로 정의되어 있던 17개의 순수 Python 헬퍼 함수가 신규 `fns_utils.py` 형제 모듈로 분리됨. 이 모듈은 PySide6, TRANSLATIONS, ConfigManager 등 어떤 의존성도 가지지 않음. 이전된 함수들: 숫자 / 자연 정렬 유틸리티 (`_pad`, `extract_number`, `_get_leading_num`, `extract_number_auto`, `auto_width_for_group`, `detect_common_prefix`, `natural_sort_key`, `_SKIP_FILES`); Tag Editor 핵심 로직 (`remove_tag_from_name`, `_build_tag_str`, `add_tag_to_name`, `depad_name`, `apply_renames`); HTML escape / unescape 유틸리티 (`_de`, `_h2t`, `_strip_xml_illegal`, `_ex`). Text Converter 패널의 `epub_to_text` / `txt_to_epub`는 유틸리티 계층이 아닌 Text Converter 로직이므로 본체에 그대로 두고, 내부에서 사용하는 HTML 헬퍼만 `fns_utils`에서 import. 본체 라인 수는 143줄 감소 (15,662 → 15,519); 17개 헬퍼와 모듈 docstring이 187줄의 새 파일에 자리. 동작 변화는 없음 — 모든 함수가 그대로 옮겨졌고 호출 측은 PySide6 import 직후에 배치된 단일 `from fns_utils import ...` 블록을 통해 동일한 식별자를 그대로 사용. 15.6k 줄에 달하는 본체를 주제별 서브모듈로 점진적으로 분리하는 v1.1.0 모듈화 트랙의 첫 단계. 국제화 마이그레이션은 후속 단계인 테마 시스템 분리 이후로 진행되었음.
- **파일 목록 위젯들이 가상화된 base 클래스 구조로 통합됨 (v1.1.0 코드 품질 트랙)** — 5개 위젯 (`MergeFileTree`, `BulkFixerFileList`, `TextConverterFileList`, Tag Editor의 `_file_list`와 `_tree`)이 거의 동일한 `QTreeWidget` 패턴을 공유하면서도 복사-붙여넣기 과정에서 미묘하게 분기되어 있었습니다 — drag-drop 로직, `_PATH_ROLE` 대응, `_make_item` / 정렬 / 중복 제거 코드가 같은데도 각각 독립적으로 유지되던 상태. 이를 3개의 신규 base 클래스로 통합하여, v1.0.11에서 Batch Renamer 미리보기에만 적용했던 `QAbstractTableModel` 가상화를 다섯 위젯 모두에 한 번에 적용: (1) `FileListModel(QAbstractTableModel)` — 범용 평면 목록 모델로, 서브클래스가 `COLUMNS = [(label_key, render_fn_or_None), ...]`를 제공하면 그에 따라 컬럼을 렌더링. `PATH_ROLE = UserRole+4`는 기존 컨벤션과 일치. Qt의 `setSortingEnabled(True)` 통합을 위한 `sort()` / `sort_key()` 메서드 보유. `data()`에서 `isinstance(item, str)` 가드를 두어, 서브클래스가 path 대신 튜플을 저장해도 기본 렌더링이 안전하게 동작 (`_TagPreviewTree`가 `(folder, original, new)` 3튜플을 직접 렌더링하는 데 사용); (2) `FileListBase(QTableView)` — 클래스 속성 (`COLUMNS`, `COLUMN_WIDTHS`, `SELECTION_MODE`, `SORT_ENABLED`, `INITIAL_SORT_COLUMN`, `INITIAL_SORT_ORDER`)으로 서브클래스 계약을 표현, 공통 메서드 (`add_files` / `remove_selected` / `clear_files` / `move_selection` / `keyPressEvent` / `retranslate_headers` / 확장자 필터링용 `_file_filter` 훅 / `@property files`)를 한곳에 통합; (3) `DragDropMixin` — drag-drop이 필요한 3개 서브클래스를 위한 mixin. `ACCEPT_EXTERNAL_DROPS` 클래스 속성 (기본 False; `MergeFileTree`만 True). v1.0.6의 `Qt.DropAction.CopyAction` 강제 패턴을 그대로 보존하여 Qt MoveAction 자동 삭제 버그 차단. `_handle_internal_drop`은 QTreeWidget의 item-rect 로직을 QTableView의 index-rect 로직으로 이식. 서브클래스 매핑: `_TagFileList` (FileListBase only, Qt 기본 정렬, ExtendedSelection); `_TagPreviewTree` (FileListBase only, SingleSelection, 정렬 비활성, 신규 `_TagPreviewModel`이 컬럼 2를 `Qt.ForegroundRole = ACCENT`로 페인트); `BulkFixerFileList` (DragDropMixin + FileListBase, `_file_filter`가 `.txt`만 허용); `TextConverterFileList` (DragDropMixin + FileListBase, 모드별 필터 — `txt2epub`은 `.txt`, `epub2txt`는 `.epub`. `set_mode()`가 모드 전환 시 목록을 비움); `MergeFileTree` (`ACCEPT_EXTERNAL_DROPS=True` DragDropMixin + FileListBase. 신규 `_MergeFileTreeModel`이 `Qt.UserRole + 1/2/3`을 외부 dict로 라우팅하여 `MergeEncodingDelegate`가 인코딩 뱃지 / 신뢰도 / 줄 수를 행별 `setData` 호출 없이 렌더링. `set_metadata_maps()`가 panel의 dict를 in-place로 연결, `refresh_path(path)`가 단일 path 메타데이터 변경 시 view에 통지). `TextMergerPanel`과 Tag Editor / Bulk Fixer / Text Converter 패널의 호출자도 함께 정리: Tag Editor의 `_file_paths`가 `list[(dirname, basename)]` 튜플에서 `list[str]` 전체 path로 전환 (중복 캐시 `_file_paths_set`은 제거됨 — `FileListBase`가 `if p not in self._files`로 중복 검사를 직접 처리하므로 별도 set이 불필요); `TextMergerPanel.file_list`는 `_tree._files`와 동일 참조로 통합되어 panel 측 변경이 모델과 항상 동기화; `_add_file_paths` / `_sort_files` / `_refresh_file_list`의 행별 `QTreeWidgetItem` 생성 루프는 `_files.extend(...)` + 단일 `_model.refresh()` 호출로 축소. 선택 시그널도 마이그레이션: `currentItemChanged` / `itemSelectionChanged` → `selectionModel().currentChanged` / `selectionModel().selectionChanged` (QTableView 등가). 헤더 i18n 갱신은 `setHeaderLabels(...)` 대신 `retranslate_headers()`로 라우팅. 순효과: 약 350줄의 서브클래스 중복 코드가 270줄의 공유 base로 대체되었고, 5개 위젯 모두에 가상화 렌더링이 일관되게 적용 — 큰 파일 목록도 어느 탭에서든 화면에 보이는 행만 렌더링됨.
- **사이드바 카드 시각적 분리감 강화 (v1.1.0 디자인 폴리싱)** — Batch Renamer 우측 사이드바 카드들 (// 번호 리셋 / // 시작 번호 / // 자릿수 / // 접두사·접미사 등)이 살짝 떠 있는 형태로 시각적으로 그룹핑되도록 개선. 정보 밀도는 그대로 유지. 세 가지 협조 변경: (1) 신규 모듈 레벨 헬퍼 `_apply_card_shadow(widget)`이 모든 `QGroupBox`에 부드러운 `QGraphicsDropShadowEffect` 적용 (blur radius 18, Y offset 3, 색은 `BORDER`에서 alpha 0.5로 derive); (2) 글로벌 `QGroupBox` 룰에 `margin-bottom:6px`와 강화된 padding (`12px 10px 10px 10px`)을 추가하여 인접 카드 사이 여백 확보; (3) `QGroupBox::title` 색이 `MUTED`에서 `ACCENT`로 변경되어 카드 헤더가 페이드된 캡션이 아닌 섹션 마커처럼 읽힘. 신규 `AppSuite._apply_card_shadows()` 메서드는 `findChildren()`로 모든 `QGroupBox` descendants를 찾아 `__init__`의 `_build()` 직후와 `apply_theme()` 마지막에 호출되어 라이트/다크 테마 변경 시 그림자 색이 `BORDER`를 따라 갱신됨. 튜닝은 두 번 만에 마무리 — 첫 시도 (blur 12, alpha 0.35)는 시각적 인지가 약해 한 단계 강화.
...
### Fixed
- **Text Merger 선택 라벨 레이아웃 깨짐 (다중 파일 선택 시)** — 하단 선택 요약 라벨 (`_lbl_selection`)이 이동/삭제 버튼과 같은 가로 행을 공유하고 있어, 다수의 파일이 선택되면 라벨의 길이가 좌측 패널의 최소 너비를 키워 우측 (// 저장 설정) 패널을 화면 밖으로 밀어내고 이동/삭제 버튼 텍스트도 잘리는 현상이 있었습니다. 수정 방법: (1) 라벨을 `bot_btn_row` 아래의 별도 세로 행으로 이동, (2) 새 `_ElideLabel` 헬퍼(아래 Added 참고)로 감싸서 가로 `minimumSize`를 무시하게 하여 긴 문자열이 부모를 부풀리지 않도록 하고, 너비가 내용보다 좁으면 `'...'`로 우측 절삭. 전체 선택 텍스트는 툴팁으로 그대로 확인 가능. v1.0.11부터 존재하던 문제이며, v1.1.0 (la-B-1.5-B) base 클래스 리팩터링의 회귀가 아님이 확인됨.

### Added
- **`_ElideLabel(QLabel)` 헬퍼 위젯** — 너비가 내용보다 좁으면 텍스트를 `'...'`로 우측 절삭하는 재사용 가능한 라벨. 전체 텍스트를 `_full_text`에 보관하고 `resizeEvent`에서 절삭을 재적용하여 부모 너비 변화에 따라 절삭 위치가 동적으로 갱신됨. 가로 `QSizePolicy`를 `Ignored`로 설정하여 긴 문자열이 부모의 최소 너비를 키우는 일이 절대 없도록 함. 모든 `setText()` 호출에서 전체 텍스트를 툴팁으로 노출. 현재는 Text Merger의 선택 요약 라벨에서 사용 중이며, 이후 가용 너비보다 콘텐츠가 길어질 수 있는 모든 상태 라벨에 적용 가능.

### Tests
- **Qt Linguist 런타임용 테스트 스위트 마무리** — `TRANSLATIONS` 딕셔너리 literal에 대한 source-grep 테스트 전부 제거 (Qt Linguist 마이그레이션 후 dict가 Python 소스에 더 이상 존재하지 않음). 동등한 커버리지는 `TestV105RegressionModule` (`self.tr()` 호출을 통한 런타임 번역 커버리지), `TestStatusRetranslate` (`QTranslator` 교체 후 상태바 `retranslate` 체인), 그리고 재작성된 `TestTranslationCompleteness` (테스트 런타임에 활성 `QTranslator`에서 번역을 직접 로드하는 13개 대분류 검증)에 위치. 번역 레지스트리 분리 단계에서 도입되었던 탐색적 `TestFnsTranslations` 클래스는 `fns_translations.py` 모듈과 함께 제거됨. 마이그레이션 과정에서 발생한 28개 source-grep 실패는 hybrid cleanup 패스로 해결 — 일부는 새 Qt Linguist API에 맞게 재작성, 일부는 동등한 런타임 커버리지가 다른 곳에 있어 제거. 총계: **한림 환경 513개 통과** (628 → 513, -115), 0 failures / 0 errors / 5 intentional skips.
- **`TestFnsTheme` (+8) — 테마 시스템 분리 모듈 트랙 회귀 검증** — `fns_theme.py`의 분리된 모듈 표면에 대한 source-grep 검증: `THEMES` dict가 9개 테마 모두를 테마별 18개 키와 함께 선언; `_hex_rgba(hex, alpha)`가 Qt 스타일시트가 소비할 수 있는 `rgba(...)` 문자열 반환; `_combo_arrow_url(color)`가 `QComboBox::down-arrow`에서 사용되는 data-URL SVG 화살표를 생성; `_detect_system_theme()`가 `QApplication.styleHints().colorScheme()`에서 `'light'` / `'dark'`로 안전 fallback과 함께 해석; `_resolve_theme(name)`이 `_detect_system_theme()`을 통해 `'auto'`를 구체 테마로 collapse; `make_style(theme)`가 9개 테마 팔레트 모두가 substitute된 전체 QSS 문자열 반환; `_apply_card_shadow(widget, border_color)`가 명시적 인자에서 그림자 색을 derive (`BORDER` global 의존을 풀어준 v1.1.0 시그니처 변경). `FileNexusSuite.py`의 인라인 `THEMES` 정의를 파싱하던 기존 source-grep 테스트 두 자국 (`TestV010Regression.test_themes_count`, `TestV010Regression.test_themes_names`)과 `TestSidebarCardPolish`의 인라인 그림자 튜닝 검증 5개가 테마 시스템 분리 단계에서 제거 — 동등한 커버리지가 `TestFnsTheme`로 이전됨. 테마 시스템 분리 변화: 625 → 625 통과 (-7 inline source-grep + 8 신규 module-track = +1, 한 통합으로 redundancy 제거하여 -1).
- **`TestTranslationCompleteness` 재작성 — source-grep과 깨끗한 단절** — 13개 대분류 검증(3 base invariant + TEST_MANAGEMENT_POLICY §5의 10 기능 영역 invariant)은 보존하되 구현은 통째 재작성: `FileNexusSuite.py`를 5-패턴 정규식으로 파싱하던 옛 `_get_lang_keys(lang)` 헬퍼는 `setUpClass`가 활성 `QTranslator`를 통해 번역을 로드하고 언어별 키 셋을 반환하는 단일 `_keys(lang)` 헬퍼로 교체. 10개 대분류 메서드에 중복되어 있던 언어별 assertion 루프를 통합하는 `_assert_keys_in_all_langs(keys, category)` 신규 헬퍼 도입. `test_zh_cn_fallback_to_zh_tw`는 source-grep arm 보존 — 데이터 invariant(`zh_tw`가 `zh_cn`에 없는 키 보유) + `_t()`와 `_rt()`가 모두 fallback 줄을 유지하는 source invariant 검증, 이는 국제화 마이그레이션의 설계상 두 함수 모두 본체에 그대로 두기 때문.
- **626 → 625 passing on Hanrim's environment** — 테마 시스템 분리는 테스트 표면을 본질적으로 평탄하게 유지(-7 inline + 8 신규 = +1, 한 통합으로 -1 netting). 67 클래스 (+1, `TestFnsTheme` 추가), 0 failures / 0 errors / 5 intentional skips.
- **626 통과 유지 (한림 환경)** — 헬퍼 함수 분리는 기존 테스트 표면을 그대로 보존. source-grep 테스트들 (`TestFileListBaseRefactor.test_sort_key_uses_natural_sort_key` 등)은 이전된 함수들이 원래 호출 위치에서 `from fns_utils import ...` 블록을 통해 동일한 식별자로 참조되므로 그대로 통과; 순수 함수 테스트들 (`TestDe`, `TestEx`, `TestNaturalSort`, `TestRemoveTag`, `TestAddTag`, `TestDepad`, `TestDetectPrefix`, `TestExtractNumber` 등 10개 클래스 126개 테스트)은 테스트 파일에 헬퍼 사본을 자체적으로 정의하므로 본체의 함수 위치에 영향받지 않음.
- **620 → 626 passing on Hanrim's environment** (+6), 66 classes (+1), 0 failures / 0 errors / 5 intentional skips
- **`TestTextMergerSelectionLayout` (+6)** — 선택 라벨 레이아웃 수정과 신규 `_ElideLabel` 헬퍼에 대한 source-grep 검증. 검증 항목: `_ElideLabel(QLabel)` 클래스 정의; `setSizePolicy(...)`로 적용되는 `QSizePolicy.Ignored` (부모 최소 너비 inflation 차단); `_full_text` 보관 및 `setToolTip` 전파를 수행하는 `setText` override; `resizeEvent` override와 짝을 이루는 `_apply_elide`가 `Qt.TextElideMode.ElideRight`로 `fontMetrics().elidedText(...)` 사용; `_lbl_selection`이 일반 `QLabel(...)` 대신 `_ElideLabel(...)`로 인스턴스화; `_lbl_selection`이 더 이상 `bot_btn_row`에 추가되지 않고 `ll.addWidget`을 통해 별도 행에 배치되었다는 negative assertion.
- **609 → 620 passing on Hanrim's environment** (+11), 65 classes (+2), 0 failures / 0 errors / 5 intentional skips
- **`TestFileListBaseRefactor` (+6)** — 3개 신규 base 클래스에 대한 source-grep 검증: 각 클래스 정의가 예상 base 클래스로 존재 (`QAbstractTableModel` / `QTableView` / mixin); `FileListModel.data()`가 `isinstance(item, str)` 가드를 두어 path가 아닌 item도 기본 렌더링이 안전; `FileListModel.PATH_ROLE`이 `UserRole+4`로 선언 (legacy `_PATH_ROLE`과 동일); `sort()` / `sort_key()` 메서드가 `natural_sort_key`를 사용; `FileListBase`가 6개 계약 속성과 모든 핵심 메서드 (`add_files` / `remove_selected` / `clear_files` / `move_selection` / `keyPressEvent` / `retranslate_headers` / `_file_filter` / `_setup_view` / `_make_model`) + `files_changed = Signal(int)` 시그널 + `files` property 보유; `DragDropMixin`이 `ACCEPT_EXTERNAL_DROPS = False` (override 가능한 기본값) + 6개 drag-drop 메서드 + `Qt.DropAction.CopyAction` 강제 (Qt MoveAction 자동 삭제 버그 차단을 위한 v1.0.6 패턴) 보유.
- **`TestFileListSubClassRefactor` (+5)** — 5개 위젯이 올바르게 리팩터링되었는지 source-grep 검증: `_TagFileList(FileListBase)` (DragDropMixin 사용하지 않음, `SORT_ENABLED=True`, 2개 i18n 컬럼 키); `_TagPreviewTree(FileListBase)` (DragDropMixin 사용하지 않음, `SELECTION_MODE=SingleSelection`, `SORT_ENABLED=False`, folder/orig/new 3개 컬럼 키, body에 `Qt.ForegroundRole`을 포함하는 `_TagPreviewModel` 사용); `BulkFixerFileList(DragDropMixin, FileListBase)` (올바른 base 순서, `ACCEPT_EXTERNAL_DROPS=False`, `_file_filter` override가 `.txt` 허용, PySide6 mixin 계약에 따라 `files_dropped`와 `order_changed` 시그널이 concrete 클래스에 선언); `TextConverterFileList(DragDropMixin, FileListBase)` (올바른 base 순서, `set_mode()`와 `_file_filter` 보유, `.epub`과 `.txt` 양쪽 참조); `MergeFileTree(DragDropMixin, FileListBase)` (올바른 base 순서, `ACCEPT_EXTERNAL_DROPS=True`, `SORT_ENABLED=False`, `set_metadata_maps()`와 `refresh_path()` 보유, body에 인코딩 `UserRole` offset 3개 — `+1` / `+2` / `+3` — 라우팅이 있는 `_MergeFileTreeModel` 사용).
- **`TestSidebarCardPolish` (+6)** — 세 가지 협조 카드 폴리싱 변경에 대한 source-grep 검증. 검증 항목: `QGraphicsDropShadowEffect` import, `_apply_card_shadow` 헬퍼 정의, 튜닝된 그림자 파라미터 (blur 18 / offset (0,3) / alpha 0.5 / `QColor(BORDER)`), `AppSuite._apply_card_shadows()` 메서드 (반드시 `findChildren(QGroupBox)` 사용, `_apply_card_shadow(gb)` 호출, `__init__`와 `apply_theme` 양쪽 최소 두 곳에서 호출 — 테마 변경 시 그림자 갱신을 보장하기 위함), `QGroupBox::title`이 `MUTED` 대신 `ACCENT` 사용, 강화된 `QGroupBox` margin / padding (`margin-bottom:6px`, `padding:12px 10px 10px 10px`).
- **594 → 603 passing on Hanrim's environment** (+9), 62 classes (+1), 0 failures / 0 errors / 5 intentional skips
...
- **`test_scenario_b_imports_present` 갱신** — `QProgressDialog`가 imports 검증에서 제거 (v1.1.0에서 `_build_progress_dlg` 헬퍼로 대체됨).
- **`test_bulk_fixer_sort_btn_uses_t` 갱신** — 검증이 `setHeaderLabels` (legacy QTreeWidget 패턴)에서 `BulkFixerFileList.COLUMNS`에 선언된 i18n 키 (신규 FileListBase 패턴)로 마이그레이션. 하드코딩 `"파일명"` 거부 검사는 그대로 유지.
- **`test_merger_has_tree_header_retranslate` 갱신** — 검증이 `setHeaderLabels` (legacy)에서 `retranslate_headers` (헤더 재렌더링을 위해 `FileListModel.headerDataChanged`를 트리거하는 신규 메서드)로 마이그레이션.
...
### Tests
- **535 → 538 passing**, 57 classes, 실패/에러/스킵 0건 (한림 PC 검증) — 누적 양식: Phase 2a/2b/3a 중간 자국 +54 후 아래 Phase 2a 본받음 자국에서 -51 (in-test mock + 12개 의존 class 메스질)
- **TestFnsUtils 본받음 자국 (+45) — Phase 2a regression test class** — v1.1.0 모듈화 트랙 본받기 (TEST_MANAGEMENT_POLICY §4.5)에 본받음 — 분리된 모듈마다 모듈 표면, source-grep invariant, 메인 모듈 통합을 검증하는 전용 `TestFns<Module>` class 추가. Phase 2a 본받음 자국이 옛 분리 자리에서 보류된 자국이었는데, 본 commit으로 `fns_utils.py`가 expose하는 17개 이름 (HTML entity decoding, XML escape, natural-sort 패딩, 태그 조작, depadding, 일괄 이름 변경) + 함수별 empty input 경계 + PySide6/TRANSLATIONS 의존 없음 source-grep invariant + 메인 모듈 import / inline 정의 없음 invariant를 검증하는 45개 자국 박힘. 옛 in-test mock 본받기 11개 class (`TestDe` / `TestEx` / `TestStripXmlIllegal` / `TestH2t` / `TestNaturalSort` / `TestRemoveTag` / `TestAddTag` / `TestDepad` / `TestDetectPrefix` / `TestExtractNumber` / `TestDepadDateRegression` — 138개 자국)는 *옮김 양식*이 아닌 *통째 메스질* 양식 본받기 — mock 시그니처/동작이 fns_utils 본질과 다른 양식이었던 자국 (예: `_de(None) → None` mock vs `AttributeError` 실제, `_ex("'") → &#39;` mock vs `&apos;` 실제, `extract_number → int|None` mock vs 패딩 문자열 실제)이라 옮김 양식 박으면 *데드 본질이 정정 양식으로 그대로 떠안기는 자국* 부담. 정책 §4.4 (모듈 분활로 본질이 깨진 source-grep 테스트 잔존 금지; 제거된 기능의 invariant 잔존 금지)에 위배되는 양식이라 메스질 양식. `TestBoundaryValues`의 empty input 자국 3개 (`depad` / `remove_tag` / `extract_number`)는 fns_utils 본받기 정정 양식으로 새 class에 흡수. `TestFilesystemIntegration` 2개 + `TestPerformance` 1개는 *Integration / Performance 본질*이 분리된 모듈 자국에서도 양호한 양식이라 fns_utils API 본받기 (`depad_name` / `remove_tag_from_name` / `natural_sort_key`) 정정 양식. 자국 변동: 138개 통째 메스질 - 45개 신규 = -93, +3개 흡수 자국 = 옛 peak 628개 자국 본받기로 -90 자국. 0 failures / 0 errors. Phase 2a 본받음 자국 마감 자국. `TestFnsTheme` (Phase 2b) + `TestFnsTranslations` (Phase 3a) class는 옛 자리에서 박힌 자국 본받음.
