...
### Changed
- **사이드바 카드 시각적 분리감 강화 (v1.1.0 디자인 폴리싱)** — Batch Renamer의 우측 사이드바 카드들(// 번호 리셋 / // 시작 번호 / // 자릿수 / // 접두사·접미사 등)이 살짝 떠 있는 자국으로 박혀 시각적으로 그룹핑되는 양식. 정보 밀도는 그대로 유지. 세 가지 협조 양식 자국: (1) 신규 모듈 레벨 헬퍼 `_apply_card_shadow(widget)`이 모든 `QGroupBox`에 부드러운 `QGraphicsDropShadowEffect`를 적용 (blur radius 18, Y offset 3, 색은 `BORDER`에서 alpha 0.5로 derive); (2) 글로벌 `QGroupBox` 룰에 `margin-bottom:6px`와 강화된 padding (`12px 10px 10px 10px`)을 추가하여 인접 카드 사이 호흡 자리 박음; (3) `QGroupBox::title` 색이 `MUTED`에서 `ACCENT`로 정착되어 카드 헤더가 *섹션 마커* 양식으로 박힘. 신규 `AppSuite._apply_card_shadows()` 메서드는 `findChildren()`로 모든 `QGroupBox` descendants를 찾아 `__init__`의 `_build()` 마감 자리와 `apply_theme()` 마감 자리 두 곳에서 호출되어 라이트/다크 테마 변경 시 그림자 색이 `BORDER` 자국 따라 갱신 양식. 튜닝은 두 호흡으로 마감 — 첫 양식 (blur 12, alpha 0.35)은 시각적 인지 자국이 약해 한 단계 강화 양식.
...
### Tests
- **603 → 609 passing on Hanrim's environment** (+6), 63 classes (+1), 0 failures / 0 errors / 5 intentional skips
- **`TestSidebarCardPolish` (+6)** — 세 가지 협조 카드 폴리싱 자국에 대한 source-grep 검증. 검증 항목: `QGraphicsDropShadowEffect` import, `_apply_card_shadow` 헬퍼 정의, 튜닝된 그림자 파라미터 (blur 18 / offset (0,3) / alpha 0.5 / `QColor(BORDER)`), `AppSuite._apply_card_shadows()` 메서드 (반드시 `findChildren(QGroupBox)` 사용 / `_apply_card_shadow(gb)` 호출 / `__init__`와 `apply_theme` 양쪽 최소 두 자리 호출 양식 — 테마 변경 시 그림자 갱신 보장), `QGroupBox::title`이 `MUTED` 대신 `ACCENT` 사용, 강화된 `QGroupBox` margin / padding (`margin-bottom:6px`, `padding:12px 10px 10px 10px`).
