...
사용자가 EPUB→TXT 변환 결과물의 용량이 원본 EPUB보다 2배 이상 커지는 현상을 제보하며 시작된 자리. 실제 조사는 두 갈래로 나뉘었다 — 하나는 스펙을 벗어난 손상 EPUB에 대한 방어 결함(spine이 비HTML 리소스를 직접 참조할 때 그 바이너리가 텍스트로 잘못 디코딩되는 문제)이었고, 다른 하나는 조사 끝에 버그가 아닌 것으로 밝혀진 포맷 자체의 특성(EPUB은 zip 압축 저장, TXT는 비압축 — 정상 EPUB이라면 변환 후 용량이 커 보이는 게 당연함)이었다. 전자만 수정 대상이었고, 겸사겸사 TXT→EPUB의 zip 압축 레벨도 최댓값으로 올렸다.

### Fixed
- **`epub_to_text()` — spine 항목의 media-type 미검증으로 인한 바이너리 오디코딩 방지** — manifest `<item>` 파싱 시 `media-type` 속성을 함께 읽어, spine 순회 단계에서 `application/xhtml+xml` / `text/html` / `text/x-oeb1-document`가 아닌 항목(이미지 등 비HTML 리소스가 spine에 직접 걸린 손상된 EPUB의 경우)을 건너뛰도록 수정. media-type 속성이 없는 항목은 확장자(`.html`/`.xhtml`)로 폴백 판단. 기존에는 이런 항목의 바이너리 데이터를 `decode("utf-8","replace")`로 강제 디코딩해, 손상된 바이트가 U+FFFD 치환 문자(UTF-8 3바이트)로 부풀려지며 결과 텍스트 용량이 불필요하게 커지고 있었음.

### Changed
- **`txt_to_epub()` — zip 압축 레벨 명시** — `zipfile.ZipFile` 호출에 `compresslevel=9`를 명시해, zlib 기본값(레벨 6 상당) 대신 최대 압축을 사용하도록 변경. 실측 절감폭은 미미(~0.75%)하나 비용 없는 개선이라 반영.

---

## [1.1.2] — 2026-05-14

Text Merger에 MD 출력 형식 선택 기능이 추가된 자리. 기존 `.txt` 고정 출력에서 `.txt` / `.md` 라디오버튼 선택으로 확장되며, `.md` 출력 시 파일 구분선이 `## 파일명` 헤더 + `---` 마크다운 양식으로 자동 전환된다. 내부적으로는 앱 아이콘 base64 데이터(`_APP_ICON_B64`, 1,090줄)가 `fns_icon.py`로 분리되어 메인 모듈 규모가 줄었다.

### Added
- **Text Merger — MD 출력 형식 추가** — 저장 설정 영역에 `출력 형식: TXT / MD` 라디오버튼이 추가됨. MD 선택 시 기본 파일명 `merged.md`, 저장 필터 `Markdown Files (*.md)`, 파일 구분선 양식 `## 파일명\n\n내용\n\n---`으로 자동 전환. 구분선 체크박스를 끄면 형식 무관하게 구분선 없이 병합되는 기존 동작 유지. 출력 형식 선택은 설정으로 저장·복원됨. 5개 언어 번역(`Output Format:`) 추가.

### Internal
- **`fns_icon.py` 분리** — 메인 모듈에 인라인으로 존재하던 앱 아이콘 base64 데이터(`_APP_ICON_B64`, 1,090줄)를 `fns_icon.py`로 분리. PySide6 의존 없는 순수 데이터 파일. 메인 모듈은 `from fns_icon import _APP_ICON_B64`로 참조. 사용자 가시 변화 없음.
