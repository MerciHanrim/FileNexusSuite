...
- **테스트 스위트 영문화 (트랙 F)** — `test_file_nexus.py`의 한국어 주석·docstring을 영문으로 변환. 277 매핑을 3개 청크에 걸쳐 적용 (F1 L1–L3500: 90 매핑, F2 L3501–L4100: 120 매핑, F3 L4101–L4590: 67 매핑). 한글 줄 1,145 → 560 (-585줄 영문화). **보호 결 459줄 엄격 보존**: `SAMPLE_KO_*` 테스트 입력 변수, assertEqual expected output, `_run_fix` / `remove_tag` / `add_tag` 등 함수 호출 인자의 한국어 문자열 — 이들은 `_SENT_END = '.!?…。！？‥」』）)}>]"\''` (ASCII + CJK 부호) 결의 *언어 중립 동작*을 검증하는 자국이라, 한국어 입력 자체가 *테스트 시나리오*. 영문 주석 안 한국어 예시 인용 9곳도 보존 (OCR 깨짐 `"휘\n둥그레"`, 단어 분할 `"참\n가자들" → "참가자들"`, 챕터 헤더 `"테스트 샘플"`, 범위 표기 `001-197화`, cp1252 깨짐 `'한'`, 정규식 템플릿 `"{n}개 파일 추가됨..."`) — 한국어 텍스트 자체가 *테스트의 증거 결*. 테스트 러너 결과 출력 D_print 5줄 (`Total tests / Passed / Failed / Errors / Skipped`)은 영문 단독 결로 변환 — 사용자 노출 UI가 아닌 CI/컨트리뷰터 노출 콘솔 출력 결. Hanrim 호칭 정합 결로 3곳 변환 (L3806 / L3881 / L4532: `신용우님` + `한림 로컬` → `Hanrim`) — v1.0.8 §4.36 매체별 표기 정책 적용. 청크별 + 전체 파일 `ast.parse` 통과 + 작업 폴더·Git 폴더 양쪽 ×2에서 535/0/0/0 검증 마감.
...
- **중국어 사전 정합 결** — `zh_tw` 사전 안에 간체 누수 결 *2줄*이 박혀 있던 자국 (`tag_drop2`, `merge_sel_one`) — 번체 UI에 간체 글자가 섞여 *문자체 정합 깨짐 결*. `zh_cn` 사전엔 `merge_sel_one` 키 자체가 *누락된 자국* — 간체 사용자는 `_t`의 zh_cn → zh_tw fallback 경로(L814)로 떨어져 *깨진 번체 사전의 간체 누수 결*을 우연히 받아안고 있던 자국 — 두 결이 서로 상쇄되어 *우연한 시각적 정합* 박힌 결. 이제 두 사전 모두 자기 문자체로 키 박힘 (`zh_tw`: `點擊以開啟資料夾選擇視窗` / `已選取: {name}` — 후자는 위 줄 `merge_sel_multi` `已選取 {n}個…` 결과 통일성 정합), *우연한 fallback 결*을 *명시적 정합*으로 바꾸는 자국.
- **Legacy `%APPDATA%` 설정 fallback 청소 (`_show_already_running_popup`)** — v1.0.x 결 통합 이전의 try-block 자국이 박혀 있었음: 먼저 `~/AppData/Roaming/FileNexusSuite/FileNexusSuite.json`(legacy 결) 결을 짚고, 없으면 `__file__`-relative 경로로 fallback. 두 결 다 사장 자국 — 코드베이스의 다른 모든 곳은 v1.0.x 이후 모듈 레벨 `_CONFIG_PATH = _app_dir() / "FileNexusSuite.json"` 결을 단독으로 사용하고 있고, legacy `%APPDATA%` 위치에 남은 사용자 0명. 본 블록도 `_CONFIG_PATH` 직접 사용 결로 통일 — 14줄 probe-and-fallback이 10줄 직접 read 결로 (-4줄, single source of truth).
...
- **8건 unclosed file handle 청소** — `test_file_nexus.py`의 `open(path, encoding='utf-8').read()` 인라인 안티패턴 8곳 — file descriptor 누수 결 + `python -W default` 결에서 `ResourceWarning` 출력. 명시적 `with open(...) as _f: ... = _f.read()` 블록 결로 변환. `TestBulkFixerFileIO` 3곳 (novel.txt / ko.txt / blank.txt) + `TestWriteEncodingReport` 5곳 (tier1 / tier2 / tier3 / truncated_count / all_languages 결, `encoding_report.txt` 자국). 560/560 그대로 유지, ResourceWarning 8 → 0.
...
- **트랙 F 학습 결을 Console Release Manual v1.3에 통합** — §5.9 보강 — *동일 문장 반복 등장 결*: Python의 `str.replace(old, new, 1)`은 첫 매칭만 치환. 동일한 docstring 문장이 여러 테스트 클래스에 반복 등장할 경우 `replace()` count 인자 없이 *전체 치환*해야 함. 본 트랙에서 v1.0.6 Phase 2-a fallback 헬퍼 설명 결이 3곳에 동일 등장하여 발견. §5.10 신규 — *영문 주석 안 한국어 예시 인용 보호 결*: 이미 영문화된 주석이 한국어를 *테스트 시나리오 자체*로 인용한 경우, 영문화 시 *테스트의 증거 결*이 사라짐. `test_file_nexus.py`에서 9곳 식별 — 메인 코드 Step 3 §1.3.3 단일 보호 18곳 결과 동일한 자국. 트랙 F 학습 결: 다중 라인 docstring 52개 (Step 3 영역의 4배) 결을 v1.2 §5.9 사전 점검 패턴 결로 처리하여 `ast.parse` 실패 0건 — 매뉴얼의 예방 가치 결을 대규모 결로 검증.
